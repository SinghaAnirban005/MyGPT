...
export function Chat({ chatId, onChatUpdate, useChatHook }: ChatProps) {
  ...
  useEffect(() => {
    ...
    const syncMessageIds = async () => {
      ...
      // Check if we have messages that might not be synced with database yet
      // Real user message IDs start with "user-" followed by timestamp
      // Real assistant message IDs are generated by the AI SDK
      // We need to sync when messages length changes to ensure we have database IDs
