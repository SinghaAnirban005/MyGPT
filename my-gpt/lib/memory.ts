...
export class ChatMemoryManager {
  ...
  async getAllMemories(): Promise<ConversationMemory> {
    try {
      ...
      memories.forEach((memory: any) => {
        ...
        if (lowerText.includes('prefer') || lowerText.includes('like') || 
            lowerText.includes('dislike') || lowerText.includes('love') || 
            lowerText.includes('hate') || lowerText.includes('favorite')) {
          preferences.push(text)
        } else if (lowerText.includes(' is ') || lowerText.includes(' are ') || 
                   lowerText.includes(' was ') || lowerText.includes(' were ') ||
                   lowerText.includes('name is') || lowerText.includes('work') ||
                   lowerText.includes('live') || lowerText.includes('age')) {
          facts.push(text)
        } else {
          context.push(text)
        }
      ...
      console.log(`Retrieved all memories for user ${this.userId}: ${facts.length} facts, ${preferences.length} preferences, ${context.length} context`)
  ...
  manageContextWindow(messages: any[]): any[] {
    ...
    console.log(`Context window managed: ${messages.length} -> ${systemMessages.length + recentMessages.length} messages`)
  ...
  async createEnrichedSystemPrompt(basePrompt: string, currentQuery?: string): Promise<string> {
    try {
      ...
      console.log(`Enriched system prompt created for user ${this.userId}: ${enrichedPrompt.length} characters`)
  ...
  async cleanupOldMemories(daysOld: number = 30): Promise<number> {
    try {
      ...
      for (const memory of memories) {
        if (daysOld === 0 || (memory.metadata?.timestamp && new Date(memory.metadata.timestamp) < cutoffDate)) {
          try {
